...
var confidenceTimeSeries = new TimeSeries();

window.onload = function() {
	var request = new XMLHttpRequest();
	// request.open("GET", "./media/whistling3.ogg", true);
	request.open("GET", "./media/001.mp3", true);
	...
	confidenceElem = document.getElementById( "confidence" );
...
function autoCorrelate( buf, sampleRate ) {
	...
	if (currentPitch == 11025) confidence = 0; // algorithm peaks at 11025 when audio isn't clear?
}
...
function updatePitch( time ) {
	...
	confidenceElem.innerText = confidence;
 	if (confidence <10) {
...
setInterval(function(){ 
	confidenceTimeSeries.append(new Date().getTime(), confidence);
	if ((isRecording || isPlaying) && confidence > 10) {
...
function createTimeline() {
	...
	// chart_gy.addTimeSeries(confidenceTimeSeries, {strokeStyle: 'pink'});
